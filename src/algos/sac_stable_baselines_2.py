...
from algos.stable_baselines_gcn_2 import CustomMultiInputExtractorActor, CustomMultiInputExtractorCritic
...
class CustomMultiInputSACPolicy(SACPolicy):
    def __init__(self, *args, **kwargs):
        super(CustomMultiInputSACPolicy, self).__init__(*args, **kwargs)
    
    def _build(self, lr_schedule):
        # Actor feature extractor
        self.actor.features_extractor = CustomMultiInputExtractorActor(self.observation_space)
        
        # Critic feature extractors
        self.critic.qf1.features_extractor = CustomMultiInputExtractorCritic(self.observation_space)
        self.critic.qf2.features_extractor = CustomMultiInputExtractorCritic(self.observation_space)
        
        super(CustomMultiInputSACPolicy, self)._build(lr_schedule)
# class CustomMultiInputSACPolicy(SACPolicy):
#     def __init__(
#         self,
#         observation_space: spaces.Space,
...
#     #     return ContinuousCritic(**critic_kwargs, hidden_features_dim=self.hidden_features_dim,
#     #                                      node_features_dim=self.node_features_dim, action_dim=self.action_dim,
#     #                                      extractor_type=self.extractor_type).to(self.device)